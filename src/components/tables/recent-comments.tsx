...
function DataTable(props: {
  ...
  const exportColumns = React.useMemo(
    () => columns.filter((c) => c.id !== "submission_link" && c.id !== "words"),
    [columns]
  );

  function presetTitleShort(preset: ExportPreset): string {
    return preset === "FOLLOW_FILTER" ? "Custom" : PRESET_LABEL[preset];
  }
  ...
  // Helper to build CSV for a specific subset (used by enhanced Export)
  function buildCSVForRows(list: Row[]) {
    const headers = exportColumns.map((c) => c.header);
    const lines: string[] = [];
    lines.push(headers.map(csvEscape).join(","));
    for (const r of list) lines.push(exportColumns.map((c) => csvEscape(getCellRaw(c, r))).join(","));
    return lines.join("\r\n");
  }
  function openPrintableTable(list: Row[], docTitle: string, rangeLabel: string) {
    if (typeof window === "undefined") return;
    const headersHtml = exportColumns.map((c) => `<th>${escapeHtml(c.header)}</th>`).join("");
    const rowsHtml = list.length
      ? list
          .map((row) => {
            const cells = exportColumns
              .map((col) => `<td>${escapeHtml(getCellRaw(col, row))}</td>`)
              .join("");
            return `<tr>${cells}</tr>`;
          })
          .join("")
      : `<tr><td colspan="${exportColumns.length}" style="text-align:center;">No rows to export</td></tr>`;
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <AlertDialogTrigger asChild>
              <Button variant="default" className="gap-2" aria-label="Export recent comments">
              ...
              <AlertDialogFooter>
                ...
                <AlertDialogAction
                  onClick={() => {
                    ...
                    const docTitle = `Recent Comments (${presetTitleShort(exportPreset)}) ${displayDate}`;
                    const rangeLabel = PRESET_LABEL[exportPreset];
                    if (exportFormat === "CSV") {
                      const csv = buildCSVForRows(data);
                      const fileName = `recent-comments-${slugify(presetTitleShort(exportPreset))}-${formatFileDate(
                        nowPH
                      )}.csv`;
                      downloadCSV(fileName, csv);
                    } else {
                      openPrintableTable(data, docTitle, rangeLabel);
                    }
